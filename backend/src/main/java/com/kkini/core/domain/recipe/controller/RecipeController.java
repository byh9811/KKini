...
public class RecipeController {
    ...
    @Operation(summary = "레시피 리스트 조회", description = "레시피 리스트를 조회하는 API입니다")
    @GetMapping
    public Response<List<RecipeListResponseDto>> getRecipeList(@ModelAttribute SearchConditionRequestDto searchConditionRequestDto) {
        List<RecipeListResponseDto> list = new ArrayList<>();
        list.add(new RecipeListResponseDto());
        list.add(new RecipeListResponseDto());
        log.debug("getRecipeList() Entered");
        log.debug("{} / {}", searchConditionRequestDto.getDifficulty(), searchConditionRequestDto.getName());
        return OK(list);
    }
    ...
    public Response<Void> addRecipe(@RequestBody RecipeRegisterRequestDto recipeRegisterRequestDto) {
        ...
        log.debug("{} / {}", recipeRegisterRequestDto.getSteps().size(), recipeRegisterRequestDto.getName());
        return OK(null);
    }
